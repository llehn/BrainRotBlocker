...
public sealed class InstallerTests : IDisposable
{
    ...
    {
        _root = Path.Combine(Path.GetTempPath(), "brb-install-tests", Guid.NewGuid().ToString("N"));
        Directory.CreateDirectory(_root);

        _sourceExe = Path.Combine(_root, "BrainRotBlocker.exe");
        File.WriteAllBytes(_sourceExe, new byte[] { 1, 2, 3, 4 }); // stand-in payload

        _testRegRoot = $@"Software\BrainRotBlockerTests\{Guid.NewGuid():N}";
        _options = new InstallOptions
        {
            ...
            UninstallKeyPath = _testRegRoot + @"\Uninstall\BrainRotBlocker",
        };
    }
    ...
    public void Install_copies_exe_and_writes_registry()
    {
        ...
        Assert.NotNull(un);
        Assert.Equal("BrainRotBlocker", un!.GetValue("DisplayName"));
