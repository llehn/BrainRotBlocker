...
internal sealed class EnforcementController : IDisposable
{
    ...
    public bool TrySaveConfiguration(EditableConfiguration editable, out string? error)
    {
        ...
        string json;
        BrainRotBlocker.Core.Configuration.BlockerConfiguration configuration;
        try
        {
            json = editable.ToJson();
            configuration = BrainRotBlocker.Core.Configuration.ConfigurationLoader.Load(json);
        }
        catch (BrainRotBlocker.Core.Configuration.ConfigurationException ex)
        {
            error = ex.Message;
            return false;
        }
