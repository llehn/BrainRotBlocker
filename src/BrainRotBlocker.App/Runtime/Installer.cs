...
internal sealed class InstallOptions
{
    ...
    public string ExeName { get; init; } = "BrainRotBlocker.exe";
    public string? AppDataDir { get; init; }

    public string RunKeyPath { get; init; } = @"Software\Microsoft\Windows\CurrentVersion\Run";
    public string RunValueName { get; init; } = "BrainRotBlocker";
    public string UninstallKeyPath { get; init; } =
        @"Software\Microsoft\Windows\CurrentVersion\Uninstall\BrainRotBlocker";

    public string DisplayName { get; init; } = "BrainRotBlocker";
    public string Version { get; init; } = "1.0.0";
    public string Publisher { get; init; } = "BrainRotBlocker";
    ...
    public static InstallOptions Default(string sourceExe)
    {
        ...
        {
            SourceExe = sourceExe,
            InstallDir = Path.Combine(local, "Programs", "BrainRotBlocker"),
            AppDataDir = Path.Combine(local, "BrainRotBlocker"),
        };
    }
}
